...
int main() {
    ...
    if (!code) {
        ...
        while (1) //서버에서도 채팅을 보낼수 있다.
        { // 무한 반복문을 사용하여 서버가 계속해서 채팅 보낼 수 있는 상태를 만들어 줌. 반복문을 사용하지 않으면 한 번만 보낼 수 있음.
            string text, msg = "";

            std::getline(cin, text); //채팅메시지를 받는다
            const char* buf = text.c_str();
            msg = server_sock.user + " : " + buf; //server_sock.user = server buf 입력값
            send_msg(msg.c_str());
        }
...
void add_client() {
    ...
    cout << new_client.sck << buf << new_client.user;
    //new_client.sck = 416, buf = 닉네임, new_client.user = 닉네임
