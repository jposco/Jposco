...
//클라이언트를 받을 수 있는 상태
// accept()
// recv() 클라이언트가 앞으로 들어올 메시지들을 받을 준비
//}

// 서버는 listen까지 잘 실행되면 소켓 활성화한다
// 서버는 accept를 통해서 클라이언트의 요청을 받음
// 새로운 소켓을 만들어서 1:1연결을 한다.(서버 - 클라이언트)
// 이때의 소켓 정보를 서버에서 sck_list vector에 모두 저장한다
// 클라이언트는 connect를 통해서 내가 연결하고자 하는 서버에 연결 요청을 보냄
// (서버에서 새로 만든 소켓을 통해, 나와 서버가 1:1 통신을 할 수 있는 상태가 됨)
// send/recv(while문으로 반복이 가능하게 처리함)
// send
// 단체채팅방을 만든다 치면 서버에 접속해있는 모든 클라이언트에게
// 한명의 클라이언트가 보낸 메시지를 전송해야함
// 누군진 모르겠지만 특정 클라이언트가 메시지를 보내면, 서버는 recv로 받음
// 서버에 연결되어있는 모든 소켓 정보(sck_list)를 이용해 for문을 돌면서 모든 클라이언트에게 send를 하게됨
// 
// 서버가 send하는 순간
// 1. accept(누가 입장했는지 공지)를 하는 순간
// 2. 서버측 콘솔창에서 채팅을 입력했을 때
// 3. 클라이언트가 보낸 메시지를 recv로 받았을때
// 4. 나갔을 때
// 
// 클라이언트가 send 하는 순간
// 1. 입장할때 connect하고 바로 한번 실행. -> 닉네임을 알려주기 위해
// 2. 콘솔에 메시지를 입력하는 순간
// 
